void drawLeadEdgeDiff(UInt_t p_ch=2,
			 ...
			 TString p_dirCalibReal="results_calib_real")
{
	...
	TString histoName;
	histoName.Form("LeadingEdgeDiff_ch%d", p_ch);
...
//		gPad->SetLogy();
	...
	TString histoTitle;
	histoTitle.Form("Leading edge difference channel %d, no fine time calibration", p_ch);
	histo[0]->SetTitle(histoTitle);
	histoTitle.Form("Leading edge difference %d, global pseudo fine time calibration", p_ch);
	histo[1]->SetTitle(histoTitle);
	histoTitle.Form("Leading edge difference %d, individual pseudo fine time calibration", p_ch);
	histo[2]->SetTitle(histoTitle);
	histoTitle.Form("Leading edge difference %d, real fine time calibration", p_ch);
	histo[3]->SetTitle(histoTitle);

	TString outFilename;
	outFilename.Form("pictures_LeadEdgeDiff/LeadEdgeDiff_noLog_ch%d.png", p_ch);
	canv->SaveAs(outFilename);
	outFilename.Form("pictures_LeadEdgeDiff/LeadEdgeDiff_noLog_ch%d.eps", p_ch);
	canv->SaveAs(outFilename);
